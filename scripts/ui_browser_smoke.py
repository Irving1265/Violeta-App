...
def create_user(username: str, *, verified: bool = True, password: str = 'Password123') -> int:
    with app.app_context():
...
def run_publish_flow(page, user_id: int) -> None:
    ...
    page.locator('.pc-category-option', has_text='Zonas inseguras').click()
    expect(page.locator('#postCreateNextBtn')).to_be_enabled(timeout=10000)
    page.locator('#postCreateNextBtn').click()
    ...
    assert post.publish_at is not None and post.publish_at > datetime.now(), 'El post no quedo en pendiente segura'
